...
/** IAU 2000B 77 项章动（保留用于对比）。入参 tt 为 J2000 起算的 TT 日数。返回 {dpsi, deps} 单位为角秒。 */
export function iau2000bLegacy(tt: number): { dpsi: number; deps: number } {
  const t = tt / 36525;
  const fa = [
    ((485868.249036 + t * 1717915923.2178) % ASEC360) * ASEC2RAD,
    ((1287104.79305 + t * 129596581.0481) % ASEC360) * ASEC2RAD,
    ((335779.526232 + t * 1739527262.8478) % ASEC360) * ASEC2RAD,
    ((1072260.70369 + t * 1602961601.2090) % ASEC360) * ASEC2RAD,
    ((450160.398036 - t * 6962890.5431) % ASEC360) * ASEC2RAD,
  ];
  let dp = 0, de = 0;
